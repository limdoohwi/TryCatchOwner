...
<body class="hold-transition skin-blue sidebar-mini">
	<c:forEach items="${mycommunity_like}" var="mycommunitylike">
		<c:if
			test="${mycommunitylike.community_no == communityDTO.community_no}">
							...
							<div class="mailbox-controls with-border text-center">
								<div class="btn-group">
									...
									<button type="button" class="btn btn-default btn-sm"
										...
										<div class="Book-Mark-Before"
											style="<c:if test='${community_like != null}'>display: none</c:if>">
	...
	<script>
		...
		$(document)
				.on(
						...
						function community_reply_insert(e) {
							if (e.keyCode == 13) {
								var community_reply_comment = $(
								...
								$
										.ajax({
											...
											data : {
												community_reply_comment : community_reply_comment,
												community_no : community_no
											},
											datatype : "json",
											success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					$.each(data.communityreplylist, function(index, replelist) {
						var replyhtml = '<span class="username">'
								+ replelist.community_reply_writer + '<br/>'
								+ replelist.community_reply_comment
								+ '<span class="text-muted pull-right">'
								+ replelist.community_reply_date + '</span>'
						'</span>';
